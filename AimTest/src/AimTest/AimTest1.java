...
public class AimTest1 extends JFrame {
    ...
    private ImageIcon BrokenButtonBasicImage = new ImageIcon(Main.class.getResource("../images/BrokenBasic.png"));
    private ImageIcon BrokenButtonEnteredImage = new ImageIcon(Main.class.getResource("../images/BrokenEentered.png"));
    // 메인화면 버튼
    ...
    private JButton BrokenButton = new JButton(BrokenButtonBasicImage);
    ...
    private void BrokenButton() {
        ...
        BrokenButton.setFocusPainted(false); // JButton이 선택 되었을떄 생기는 테두리 사용안함
        BrokenButton.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseEntered(MouseEvent e) { // 마우스가 컴포넌트 영역 안으로 들어올떄
                BrokenButton.setIcon(BrokenButtonEnteredImage);
                BrokenButton.setCursor(new Cursor(Cursor.HAND_CURSOR)); // 버튼에 마우스가 올라가면 핸드커서
            }

            @Override
            public void mouseExited(MouseEvent e) { // 마우스가 컴포넌트 영역 밖으로 나갈떄
                BrokenButton.setIcon(BrokenButtonBasicImage);
                BrokenButton.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));// 버튼에 마우스가 내려가면 기본커서
            }
